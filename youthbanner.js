...
if ($.isNode()) {
  if (process.env.YOUTHBANNER && process.env.YOUTHBANNER.indexOf('#') > -1) {
   youthBanner = process.env.YOUTHBANNER.split('#');
   console.log(`您选择的是用"#"隔开\n`)
  }
  else if (process.env.YOUTHBANNER && process.env.YOUTHBANNER.indexOf('\n') > -1) {
   youthBanner = process.env.YOUTHBANNER.split('\n');
  ...
  Object.keys(youthBanner).forEach((item) => {
        if (youthBanner[item]) {
          youthBannerArr.push(youthBanner[item])
        }
    });
